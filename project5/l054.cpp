...
void part4(int argc, char* argv[]) {
    ...
    vector<vector<double> > magnitude; vector<double> temp;
    for(int i = 0; i < x_direction.size(); i++) {
        for(int j = 0; j < x_direction[0].size(); j++) {
            temp.push_back(pow(x_direction[i][j], 2) + pow(y_direction[i][j], 2));
        }
        magnitude.push_back(temp); temp.clear();
    }
}
...
vector<vector<double> > accumulator(vector<vector<double> > array) {
    vector<vector<vector<double > > > m(array.size(), vector<vector<double> > (array[0].size(), vector<double> (1, 0)));
    int a = 0, b = 0, r = 0;
